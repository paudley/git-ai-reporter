...
class TestDateRangeBug:
    ...
    def test_end_date_commits_are_included(self, analyzer_config: GitAnalyzerConfig, mock_repo: tuple) -> None:
        ...
        assert len(commits) == 4, f"Expected 4 commits, got {len(commits)}: {commit_messages}"
        assert all(msg in commit_messages for msg in expected_messages), \
        ...
        assert "feat: Important feature on Aug 23 late evening" in commit_messages, \
            "Late evening commit on end_date should be included"
        
        # Check that Aug 25 commit is NOT included
        assert "docs: Documentation on Aug 25" not in commit_messages, \
            "Commits after end_date should not be included"

    def test_single_day_range_includes_all_commits(self, analyzer_config: GitAnalyzerConfig, mock_repo: tuple) -> None:
        ...
        assert len(commits) == 3, f"Expected 3 commits from Aug 23, got {len(commits)}: {commit_messages}"
        assert all(msg in commit_messages for msg in expected_messages), \
    ...
    def test_actual_repository_scenario(self, analyzer_config: GitAnalyzerConfig) -> None:
        ...
        assert "7702d0b" in commit_hexshas, \
            f"Allure commit (7702d0b) made at 22:48 on Aug 23 should be included. Got: {commit_hexshas}"
        
        # Both commits from Aug 23 should be there
        assert len(commits) == 2, \
            f"Expected 2 commits from Aug 23, got {len(commits)}: {[c.message for c in commits]}"